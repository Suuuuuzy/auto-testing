...
LOG_NAME = 'output/api_usage.log'
RESULT_NAME = 'output/api_usage.json'
OUTPUT_MD = 'output/api_usage.md'
API_FILE = 'input/config.json'
EVALUATION_FILE = 'input/evaluation_10k.txt'
...
def update_json_file(output, filename=RESULT_NAME):
...
def gen_md(output, checked_count):
        # Calculate the rate of miniappids for each API
    table_data = []
    for api in output:
        miniappids_count = len(output[api]["miniappids"])
        if checked_count > 0:
    ...
    all_miniappids = set()
    for api in output:
        all_miniappids.update(output[api]["miniappids"])
...
def initialize_output(apis, checked_count):
    """
    Initialize or load output from the JSON file, calculate the rate of miniappids for each API,
    and output the output in Markdown table format.
    """
    if os.path.exists(RESULT_NAME):
        with open(RESULT_NAME, 'r', encoding='utf-8') as f:
            output = json.load(f)
    else:
        output = {api: {"num": 0, "miniappids": []} for api in apis}

    gen_md(output, checked_count)
    return output
...
def main():
    ...
    for mini_app_dir in mini_app_dirs:
                        ...
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            for api in apis:
                                ...
                                if re.search(re.escape(api_reg), content):
                                    # Update the output
                                    if appid not in output[api]["miniappids"]:
                                        output[api]["miniappids"].append(appid)
                                        output[api]["num"] += 1
        ...
        update_json_file(output)
        # logging.info(f"Updated JSON file after scanning directory: {mini_app_dir}")

    print("Scanning complete. output logged in 'api_usage.log' and saved in 'api_usage.json'.")
