...
# Load config.json
    ...
    for miniappid, flows in mini_app_data.items():
        for flow in flows:
            source = 'wx.'+flow["source"].split("---")[-1].strip()
            sink = 'wx.'+flow["sink"].split("---")[-1].strip()
            # print(source, sink)
