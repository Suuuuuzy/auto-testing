...
class WXMLDocumentParser():
    ...
    def run(self, print_ast_flag : bool = False, get_json_result : bool = False) -> str | Dict[str, any]:
        ...
        if get_json_result is False:
            return_buf : List[str] = ['']
            UtilsScript.get_ast(self.m_root, return_buf)
            return return_buf[0]

        return UtilsScript.get_ast_via_json(self.m_root)
        ...
        for idx in range(ref_node.get_num_children()):
            ...
            if (ref_child is not None and ref_child.type() == NodeScript.NodeType.ATTRIBUTE_NODE):
                ...
                if (len(attribute_name) > 5 and attribute_name[:5] == "data-"):
                    ...
                    event_instance.m_target.m_dataset[attribute_name[5:]] = ref_child.get_auxiliary_data()
