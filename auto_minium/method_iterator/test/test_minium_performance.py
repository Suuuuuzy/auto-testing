...
class Minium_Query_Perf(BaseDef):
    logger_main = None
    ...
    def _setupProcessOutput(self):
        self.logger.info(f'[+] Setup output_progress')
        # Get the filename from the logger's handlers
        for handler in self.logger_main.handlers:
            if isinstance(handler, logging.FileHandler):
                self.logger.info("Log file name:", handler.baseFilename)
    ...
    def test_methods(self):
                ...
                while len(items)>0:
                    ...
                    try:
                        ...
                        if args is None:
                            ...
                            items.remove(item)
                            continue
                        if item["handler"]=="Not solved":
                            self.logger_main.info(f"[+]{item['trigger']} handler not solved")
                            items.remove(item)
                            continue
        ...
        # ======= test starts here ======= 
        time.sleep(10) # give it some time for onLaunch,this is necesseary!
        # relaunch
        cur_path = self.app.get_current_page().path
        self.app.relaunch(cur_path, {})
        time.sleep(10) # give it some time for onLaunch 
        # get_perf_time
        self.app.get_perf_time(entry_types=['render', 'script', 'navigation'])
        MAX_VISIT_TIME = 3
